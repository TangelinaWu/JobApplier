// Storage wrappers for browser.storage.local
...
async function getProfile() {
  const result = await browser.storage.local.get(StorageKeys.PROFILE);
...
async function getAppLog() {
  const result = await browser.storage.local.get(StorageKeys.APP_LOG);
...
async function appendAppLog(entry) {
  ...
  await browser.storage.local.set({ [StorageKeys.APP_LOG]: log });
}
